...
class NabiOSINTAPI:
    ...
    def yandex_search(self, query):
        ...
        try:
            ...
            results = []
            search_items = soup.find_all('li', class_='serp-item') or soup.find_all('div', class_='organic')
            ...
            for item in search_items[:5]:
                ...
                if link and link.get('href'):
                    ...
                    results.append({"title": title, "url": url, "source": "Yandex"})
...
def api_ip():
    ...
    if not address: return jsonify({"error": "IP address required"}), 400
    return jsonify(api.ip_geolocation(address))

@app.route('/api/v1/phone')
def api_phone():
    number = request.args.get('number')
    if not number: return jsonify({"error": "Phone number required"}), 400
...
def api_yandex():
    ...
    if not query: return jsonify({"error": "Search query required"}), 400
...
def api_vpn():
    ...
    if not ip: return jsonify({"error": "IP address required"}), 400
...
def api_reverseimage():
    ...
    if not image_url: return jsonify({"error": "Image URL required"}), 400
...
def api_bin():
    ...
    if not bin_number: return jsonify({"error": "BIN number required"}), 400
    return jsonify(api.bin_check(bin_number))

@app.route('/api/v1/crypto')
def api_crypto():
    address = request.args.get('address')
    if not address: return jsonify({"error": "Wallet address required"}), 400
    return jsonify(api.crypto_wallet_analysis(address))

@app.route('/api/v1/imei')
def api_imei():
    imei = request.args.get('imei')
    if not imei: return jsonify({"error": "IMEI number required"}), 400
...
def api_browser():
    ...
    if not user_agent: return jsonify({"error": "User agent required"}), 400
...
def api_telegram():
    username = request.args.get('username')
    if not username: return jsonify({"error": "Telegram username required"}), 400
    return jsonify(api.telegram_osint(username))

@app.route('/api/v1/instagram')
def api_instagram():
    username = request.args.get('username')
    if not username: return jsonify({"error": "Instagram username required"}), 400
    return jsonify(api.instagram_analysis(username))

@app.route('/api/v1/vk')
def api_vk():
    user_id = request.args.get('user_id')
    if not user_id: return jsonify({"error": "VK user ID required"}), 400
...
def api_bitcoin():
    address = request.args.get('address')
    if not address: return jsonify({"error": "Bitcoin address required"}), 400
    return jsonify(api.bitcoin_transactions(address))

@app.route('/api/v1/ethereum')
def api_ethereum():
    address = request.args.get('address')
    if not address: return jsonify({"error": "Ethereum address required"}), 400
    return jsonify(api.ethereum_analysis(address))

@app.route('/api/v1/tax')
def api_tax():
    inn = request.args.get('inn')
    if not inn: return jsonify({"error": "INN required"}), 400
...
def api_flight():
    ...
    if not flight_number: return jsonify({"error": "Flight number required"}), 400
    return jsonify(api.flight_search(flight_number))

@app.route('/api/v1/hotel')
def api_hotel():
    booking_ref = request.args.get('booking')
    if not booking_ref: return jsonify({"error": "Booking reference required"}), 400
...
def api_carplate():
    ...
    if not plate: return jsonify({"error": "Car plate required"}), 400
...
def api_driverlicense():
    ...
    if not license_number: return jsonify({"error": "Driver license number required"}), 400
    return jsonify(api.driver_license_check(license_number))

@app.route('/api/v1/passport')
def api_passport():
    passport_number = request.args.get('passport')
    if not passport_number: return jsonify({"error": "Passport number required"}), 400
...
def api_landplot():
    ...
    if not cadastral: return jsonify({"error": "Cadastral number required"}), 400
    return jsonify(api.land_plot_search(cadastral))

@app.route('/api/v1/corporate')
def api_corporate():
    company = request.args.get('company')
    if not company: return jsonify({"error": "Company name/INN required"}), 400
    return jsonify(api.corporate_data_search(company))

@app.route('/api/v1/marine')
def api_marine():
    imo = request.args.get('imo')
    if not imo: return jsonify({"error": "IMO number required"}), 400
    return jsonify(api.marine_vessel_search(imo))

@app.route('/api/v1/aircraft')
def api_aircraft():
    registration = request.args.get('registration')
    if not registration: return jsonify({"error": "Aircraft registration required"}), 400
    return jsonify(api.aircraft_search(registration))

@app.route('/health')
def health_check():
    return jsonify({"status": "healthy", "service": "NabiSystem VIP OSINT API", "timestamp": time.time()})
